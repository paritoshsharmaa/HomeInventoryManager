...
public class inentory extends JFrame
{	
	...
	// check for description
		...
		if (locationComboBox.getSelectedItem().toString().equals(locationComboBox.getItemAt(i).toString()))
		{
			...
			private String dateToString(Date dd)
			{
				...
				ImageIcon(inentory.photoTextArea.getText()).getImage();
				int w = getWidth();
				int h = getHeight();
				...
				{
				Graphics2D g2D = (Graphics2D) g;
				if ((pageIndex + 1) > inentory.lastPage)
				{
				return NO_SUCH_PAGE;
				}
				int i, iEnd;
				...
				iEnd = inentory.entriesPerPage * (pageIndex + 1);
				if (iEnd > inentory.numberEntries)
				iEnd = inentory.numberEntries;
				for (i = 0 + inentory.entriesPerPage * pageIndex; i < iEnd; i++)
				{
				// dividing line
				...
				g2D.drawString(inentory.myInventory[i].description, (int) pf.getImageableX(), y);
				y += dy;
				g2D.setFont(new Font("Arial", Font.PLAIN, 12));
				g2D.drawString("Location: " + inentory.myInventory[i].location, (int)
				(pf.getImageableX() + 25), y);
				y += dy;
				if (inentory.myInventory[i].marked)
				...
				g2D.drawString("Serial Number: " +
						inentory.myInventory[i].serialNumber, (int) (pf.getImageableX() + 25), y);
				y += dy;
				g2D.drawString("Price: $" + inentory.myInventory[i].purchasePrice + ",Purchased on:" + inentory.myInventory[i].purchaseDate, (int) (pf.getImageableX() + 25), y); 

						 
						y += dy;
				g2D.drawString("Purchased at: " +
						inentory.myInventory[i].purchaseLocation, (int) (pf.getImageableX() + 25), y);
				y += dy;
				g2D.drawString("Note: " + inentory.myInventory[i].note, (int)
				(pf.getImageableX() + 25), y);
				y += dy;
				try
				{
				...
				ImageIcon(inentory.myInventory[i].photoFile).getImage();
